...
The testing cycle covered core functional flows, user experience, credential/session security, real-time sync performance, network resilience, and monetization mechanisms to evaluate release readiness.

---

## ⚙️ Test Scope & Environment

* **Target Platform:** Android (Android 14)
* **Network Conditions:** Wi-Fi & 4G LTE (including throttled weak-network simulation)
* **Testing Type:** Manual Black-Box Functional & Non-Functional Testing
* **Tester:** Mohan Srivastav
* **Modules Covered:**
  * Authentication & Credential Security
  * Session Management & Device Security
  * Real-Time Messaging & Room Performance
  * Media Uploads & Profile Management
  * Runtime Permissions & System Notifications
  * Network Resilience & Offline Modes
  * VIP In-App Monetization
...
1. **VIP Billing Failure:** Google Play Billing verification error blocks in-app purchases (`TC-024` / `BUG-014`).
2. **Session Drop on App Backgrounding:** App force-invalidates user login session upon backgrounding (`TC-007` / `BUG-003`, `TC-018` / `BUG-010`).
3. **Session Loss on Transient Drops:** Brief network drops force immediate user logout instead of auto-reconnecting (`TC-021` / `BUG-012`).
4. **UI Freeze on Weak Connection:** Application freezes (ANR-like state) without loading indicators on slow networks (`TC-022` / `BUG-013`).
5. **No Brute-Force Rate Limiting:** Unlimited password attempts allowed without lockout or CAPTCHA (`TC-005` / `BUG-001`).
6. **No Screen Rotation Support:** Application UI remains permanently locked in Portrait mode even with OS Auto-Rotate enabled on Android 14 (`TC-030` / `BUG-018`).
...
* 📄 **[Test Plan (Word Document)](./Test_Plan.docx):** Defines scope, strategy, exit criteria, entry criteria, and risks.
...
1. **Fix Monetization & Security:** Resolve Google Play Billing verification errors and implement rate-limiting on login endpoints.
...
* **Focus:** Mobile Testing, STLC Documentation, Defect Management
